...
class FunSetSuite extends FunSuite {
  ...
  test("filter contains all elements of a set that are accepted by a given predicate p") {
    new TestSets {
      val s: Set = union(union(s1, s2), s3)
      val filtered: Set = filter(s, x => x - 3 >= 0)
  ...
  test("forall tests whether a given predicate is true for all elements of the set") {
    new TestSets {
      ...
      assert(!forall(s, x => x - 1 > 0), "Forall 4")
    }
  }
  ...
  test("exists tests whether a set contains at least one element for which the given predicate is true") {
    new TestSets {
      ...
      assert(exists(s, x => x > 0), "Forall 1")
      assert(!exists(s, x => x < 0), "Forall 2")
      assert(exists(s, x => x - 1 >= 0), "Forall 3")
      assert(exists(s, x => x - 1 > 0), "Forall 4")
      assert(!exists(s, x => x - 5 > 0), "Forall 5")
    }
  }
