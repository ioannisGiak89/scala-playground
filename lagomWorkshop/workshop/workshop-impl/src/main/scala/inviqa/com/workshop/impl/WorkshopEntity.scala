...
/**
  * This is an event sourced entity. It has a state, [[WorkshopState]], which
  ...
  */
class WorkshopEntity extends PersistentEntity {

  override type Command = WorkshopCommand[_]
  override type Event = WorkshopEvent
  override type State = WorkshopState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: WorkshopState = WorkshopState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case WorkshopState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        WorkshopState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class WorkshopState(message: String, timestamp: String)

object WorkshopState {
  /**
    ...
    */
  implicit val format: Format[WorkshopState] = Json.format
}

/**
  * This interface defines all the events that the WorkshopEntity supports.
  */
sealed trait WorkshopEvent extends AggregateEvent[WorkshopEvent] {
  def aggregateTag = WorkshopEvent.Tag
}

object WorkshopEvent {
  val Tag = AggregateEventTag[WorkshopEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends WorkshopEvent
...
/**
  ...
  */
sealed trait WorkshopCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends WorkshopCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends WorkshopCommand[String]
...
/**
  ...
  * application loader.
  */
object WorkshopSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[WorkshopState]
  )
}
